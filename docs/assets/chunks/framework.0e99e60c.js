  ...
  Hi = (e) => {
    ...
    if (t.length) {
      ...
      for (let s = 0; s < t.length; s++) {
        const r = t[s]
        ur(r) && !ar(r) ? r.delete(e) : (t[n++] = r), (r.w &= ~Ve), (r.n &= ~Ve)
      }
      t.length = n
    }
  },
...
function Ur(e, t, n = se) {
  ...
  if ((mn(t, s, n), n)) {
    let r = n.parent
    for (; r && r.parent; ) gn(r.parent.vnode) && $o(s, t, n, r), (r = r.parent)
  }
}
function $o(e, t, n, s) {
  const r = mn(t, e, s, !0)
...
const Ql = window.__VP_SITE_DATA__,
  fi = /^[a-z]+:/i,
  Uc = /^pathname:\/\//,
  jc = 'vitepress-theme-appearance',
  ui = /#.*$/,
  Gl = /(index)?\.(md|html)$/,
...
function ec(e, t, n = !1) {
  ...
  if (((e = Xs(`/${e}`)), n)) return new RegExp(t).test(e)
  if (Xs(t) !== e) return !1
  const s = t.match(ui)
  return s ? (Ce ? location.hash : '') === s[0] : !0
}
function Xs(e) {
  return decodeURI(e).replace(ui, '').replace(Gl, '')
}
function tc(e) {
  return fi.test(e)
}
...
const lc = Symbol(),
  nt = fo(Ql)
function Dc(e) {
  const t = Te(() => nc(nt.value, e.data.relativePath))
  return {
    site: t,
...
function Qs(e) {
  return fi.test(e) || e.startsWith('.') ? e : cc(nt.value.base, e)
}
function fc(e) {
  let t = e.replace(/\.html$/, '')
  if (((t = decodeURIComponent(t)), (t = t.replace(/\/$/, '/index')), Ce)) {
    const n = '/Essetialjs/'
    t = Zs(t.slice(n.length).replace(/\//g, '_') || 'index') + '.md'
    ...
    s ||
      ...
      (t = `${n}assets/${t}.${s}.js`)
  } else t = `./${Zs(t.slice(1).replace(/\//g, '_'))}.md.js`
  return t
}
let Zt = []
function kc(e) {
...
const uc = Symbol(),
  Gs = 'http://a.com',
  ac = () => ({ path: '/', component: null, data: ai })
function Wc(e, t) {
  const n = un(ac()),
    s = { route: n, go: r }
            ...
            !l.ctrlKey &&
              ...
              O === W.pathname && P === W.search
                ? L &&
                  ...
                  er(a, L, a.classList.contains('header-anchor')))
                : r(p))
          }
        },
        { capture: !0 }
...
function er(e, t, n = !1) {
  ...
  if (s) {
    ...
    else if (typeof r == 'string') i = tr(r)
    ...
    setup(e) {
      ...
      return () =>
        jn(e.as, { style: { position: 'relative' } }, [
          t.component
            ? jn(t.component, { onVnodeMounted: nr, onVnodeUpdated: nr })
            : '404 Page Not Found'
        ])
    }
  }),
